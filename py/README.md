...
| `email` | `str` | The email address that was analyzed, returned in the original format provided. |
...
| `smtp` | `Any` |  |
| `suggestion` | `str` | Suggested correction for misspelled domains. |
| `syntax` | `dict` | Detailed syntax analysis of the email address components. |
...
| Field | Type | Description |
| --- | --- | --- |
| `domain` | `str` | The normalized domain that was analyzed (lowercased, scheme/path stripped). |
...
| `resolved_ips` | `list` | DNS A/AAAA records the domain currently resolves to. |
| `threat` | `dict` | Threat-intelligence verdict for the domain itself (independent of its IPs). |
...
| `syntax` | `dict` | Detailed syntax validation results and email component breakdown. |
...
| Field | Type | Description |
| --- | --- | --- |
| `asn` | `str | None` | Autonomous System Number in AS<number> format. |
| `ip` | `str` | The IP address that was analyzed, returned in standard format. |
| `isp` | `str | None` | Internet Service Provider name derived from the ASN organization field. |
| `location` | `dict` | Geographic location and timezone information for the IP address. |
| `suspicious_factors` | `dict` | Comprehensive security threat analysis and suspicious activity indicators. |
...
| Field | Type | Description |
| --- | --- | --- |
| `email_api` | `dict` | Email validation API rate limit information |
| `interval_seconds` | `int` | Rate limit interval in seconds (time period for quota renewal) |
| `ip_api` | `dict` | IP lookup API rate limit information |
| `next_renewal_date` | `str` | Next billing/renewal date when the quota will be reset (ISO 8601 date format) |
| `plan_id` | `str` | Subscription plan ID or 'default' for free tier users |
| `plan_name` | `str` | Human-readable plan name (if available) |
| `status` | `str | None` | Subscription status (active, past_due, cancelled, etc.) |
