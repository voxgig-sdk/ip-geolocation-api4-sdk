...
class IpReputationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IpReputationEntity(self._client, opts)
    ...
    def data_get(self) -> IpReputation:
    ...
    def match_get(self) -> IpReputation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> IpReputation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IpReputation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
