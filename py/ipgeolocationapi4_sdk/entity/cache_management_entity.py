...
from ipgeolocationapi4_sdk.ipgeolocationapi4_types import (
    CacheManagement,
...
class CacheManagementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CacheManagementEntity(self._client, opts)
    ...
    def data_get(self) -> CacheManagement:
    ...
    def match_get(self) -> CacheManagement:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CacheManagement:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CacheManagement().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CacheManagement:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CacheManagement().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
