...
class TestForwardDirect:
    ...
    def test_should_direct_load_forward(self):
        setup = _forward_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-forward", "live" if setup["live"] else "unit")
        if _skip:
...
def _forward_direct_setup(mockres):
