    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IP_GEOLOCATION_API4_TEST_BATCH_ENTID JSON to run live")
        client = setup["client"]
...
def _batch_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "IP_GEOLOCATION_API4_TEST_BATCH_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "IP_GEOLOCATION_API4_TEST_BATCH_ENTID": idmap,
        "IP_GEOLOCATION_API4_TEST_LIVE": "FALSE",
        "IP_GEOLOCATION_API4_TEST_EXPLAIN": "FALSE",
    })

    idmap_resolved = helpers.to_map(
        env.get("IP_GEOLOCATION_API4_TEST_BATCH_ENTID"))
