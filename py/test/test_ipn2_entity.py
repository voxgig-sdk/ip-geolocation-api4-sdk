...
class TestIpn2Entity:

    def test_should_create_instance(self):
        testsdk = IpGeolocationApi4SDK.test(None, None)
        ent = testsdk.Ipn2(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _ipn2_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "ipn2." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IPGEOLOCATIONAPI__TEST_IPN__ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        ipn2_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.ipn2")))
        ipn2_ref01_data = None
        if len(ipn2_ref01_data_raw) > 0:
...
def _ipn2_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/ipn2/Ipn2TestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "IPGEOLOCATIONAPI__TEST_IPN__ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("IPGEOLOCATIONAPI__TEST_IPN__ENTID"))
