# IpGeolocationApi4 SDK test runner
...
class IpGeolocationApi4TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpGeolocationApi4TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpGeolocationApi4TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpGeolocationApi4TestRunner.getenv("IP_GEOLOCATION_API4_TEST_LIVE")
        override = IpGeolocationApi4TestRunner.getenv("IP_GEOLOCATION_API4_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpGeolocationApi4TestRunner.getenv(key)
        ...
        explain = IpGeolocationApi4TestRunner.getenv("IP_GEOLOCATION_API4_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_GEOLOCATION_API4_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpGeolocationApi4TestRunner._test_control is not None:
            return IpGeolocationApi4TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpGeolocationApi4TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpGeolocationApi4TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpGeolocationApi4TestRunner.load_test_control()
...
def load_env_local():
    IpGeolocationApi4TestRunner.load_env_local()


def env_override(m):
    return IpGeolocationApi4TestRunner.env_override(m)


def entity_data(v):
    return IpGeolocationApi4TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpGeolocationApi4TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpGeolocationApi4TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpGeolocationApi4TestRunner.load_test_control()


def live_client_options():
    return IpGeolocationApi4TestRunner.live_client_options()


def live_delay_ms():
    return IpGeolocationApi4TestRunner.live_delay_ms()
