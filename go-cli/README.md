# ip-geolocation-api4-cli

AQL-driven command-line client **and** interactive REPL for the IpGeolocationApi4
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-api4-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-geolocation-api4-cli --help
...
./ip-geolocation-api4-cli load 1 advanced            # {id:1} shorthand
./ip-geolocation-api4-cli load '{id:1}' advanced       # explicit match map

# 5. Override the API base URL for a single call
IP_GEOLOCATION_API4_BASE=https://api.example.com ./ip-geolocation-api4-cli load 1 advanced

# 6. No arguments -> interactive REPL
./ip-geolocation-api4-cli
ip-geolocation-api4> load 1 advanced
ip-geolocation-api4> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-api4-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-geolocation-api4-cli load 1 advanced
   ```
...
```sh
./ip-geolocation-api4-cli load 1 advanced          # scalar shorthand for {id:1}
./ip-geolocation-api4-cli load '{id:1}' advanced     # explicit match map
```
...
./ip-geolocation-api4-cli load 1 advanced
```
...
Run with no arguments to open a REPL (prompt `ip-geolocation-api4>`). Each line is
evaluated as its own AQL expression:

```text
$ ./ip-geolocation-api4-cli
ip-geolocation-api4> load 1 advanced
ip-geolocation-api4> /help
ip-geolocation-api4> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `advanced`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-geolocation-api4-cli`. |
...
(`./ip-geolocation-api4-cli <expr>`) and interactively (the REPL), and expressions compose the
