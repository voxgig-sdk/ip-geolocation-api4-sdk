// Command IpGeolocationApi4-cli is a CLI + REPL for the IpGeolocationApi4 SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_GEOLOCATION_API4_APIKEY (api key), IP_GEOLOCATION_API4_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_GEOLOCATION_API4_APIKEY carries the API key and
	// IP_GEOLOCATION_API4_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_GEOLOCATION_API4_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_GEOLOCATION_API4_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpGeolocationApi4SDK(opts)
