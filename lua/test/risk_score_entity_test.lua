...
describe("RiskScoreEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RiskScore(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = risk_score_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "risk_score." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_RISK_SCORE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local risk_score_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.risk_score")))
    local risk_score_ref01_data = nil
    if #risk_score_ref01_data_raw > 0 then
      risk_score_ref01_data = helpers.to_map(risk_score_ref01_data_raw[1][2])
    end
...
function risk_score_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/risk_score/RiskScoreTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read risk_score test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("IP_GEOLOCATION_API4_TEST_RISK_SCORE_ENTID")
