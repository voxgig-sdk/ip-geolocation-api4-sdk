-- IpGeolocationApi4 SDK BatchEmailValidationResponseDto entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BatchEmailValidationResponseDtoEntity = {}
BatchEmailValidationResponseDtoEntity.__index = BatchEmailValidationResponseDtoEntity


function BatchEmailValidationResponseDtoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BatchEmailValidationResponseDtoEntity)
  self._name = "batch_email_validation_response_dto"
  self._client = client
...
function BatchEmailValidationResponseDtoEntity:get_name()
  return self._name
end


function BatchEmailValidationResponseDtoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BatchEmailValidationResponseDtoEntity.new(self._client, opts)
end
...
function BatchEmailValidationResponseDtoEntity:mark_deleted()
  self._deleted = true
end


function BatchEmailValidationResponseDtoEntity:deleted()
  return true == self._deleted
end


function BatchEmailValidationResponseDtoEntity:data_set(args)
  if args ~= nil then
...
function BatchEmailValidationResponseDtoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BatchEmailValidationResponseDtoEntity:match_set(args)
  if args ~= nil then
...
function BatchEmailValidationResponseDtoEntity:match_get()
...
function BatchEmailValidationResponseDtoEntity:stream(action, args, callopts)
...
---@param reqdata BatchEmailValidationResponseDtoCreateData
---@param ctrl? table
---@return BatchEmailValidationResponseDto
---@return string? err
function BatchEmailValidationResponseDtoEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function BatchEmailValidationResponseDtoEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return BatchEmailValidationResponseDtoEntity
