...
local TorEntity = {}
TorEntity.__index = TorEntity


function TorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TorEntity)
  self._name = "tor"
  self._client = client
...
function TorEntity:get_name()
  return self._name
end


function TorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TorEntity.new(self._client, opts)
end
...
function TorEntity:mark_deleted()
  self._deleted = true
end


function TorEntity:deleted()
  return true == self._deleted
end


function TorEntity:data_set(args)
  if args ~= nil then
...
function TorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TorEntity:match_set(args)
  if args ~= nil then
...
function TorEntity:stream(action, args, callopts)
...
---@param reqmatch TorLoadMatch
---@param ctrl? table
---@return Tor
---@return string? err
function TorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TorEntity:_run_op(ctx, post_done)
  local utility = self._utility
