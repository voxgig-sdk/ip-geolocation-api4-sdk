...
local ApiUsageSummaryEntity = {}
ApiUsageSummaryEntity.__index = ApiUsageSummaryEntity


function ApiUsageSummaryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiUsageSummaryEntity)
  self._name = "api_usage_summary"
  self._client = client
...
function ApiUsageSummaryEntity:get_name()
  return self._name
end


function ApiUsageSummaryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiUsageSummaryEntity.new(self._client, opts)
end
...
function ApiUsageSummaryEntity:mark_deleted()
  self._deleted = true
end


function ApiUsageSummaryEntity:deleted()
  return true == self._deleted
end


function ApiUsageSummaryEntity:data_set(args)
  if args ~= nil then
...
function ApiUsageSummaryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiUsageSummaryEntity:match_set(args)
  if args ~= nil then
...
function ApiUsageSummaryEntity:match_get()
...
function ApiUsageSummaryEntity:stream(action, args, callopts)
...
---@param reqmatch ApiUsageSummaryLoadMatch
---@param ctrl? table
---@return ApiUsageSummary
---@return string? err
function ApiUsageSummaryEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ApiUsageSummaryEntity:_run_op(ctx, post_done)
  local utility = self._utility
