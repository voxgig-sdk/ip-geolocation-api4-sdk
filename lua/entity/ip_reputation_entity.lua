...
local IpReputationEntity = {}
IpReputationEntity.__index = IpReputationEntity


function IpReputationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpReputationEntity)
  self._name = "ip_reputation"
  self._client = client
...
function IpReputationEntity:get_name()
  return self._name
end


function IpReputationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpReputationEntity.new(self._client, opts)
end
...
function IpReputationEntity:mark_deleted()
  self._deleted = true
end


function IpReputationEntity:deleted()
  return true == self._deleted
end


function IpReputationEntity:data_set(args)
  if args ~= nil then
...
function IpReputationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpReputationEntity:match_set(args)
  if args ~= nil then
...
function IpReputationEntity:match_get()
...
function IpReputationEntity:stream(action, args, callopts)
...
---@param reqmatch IpReputationLoadMatch
---@param ctrl? table
---@return IpReputation
---@return string? err
function IpReputationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function IpReputationEntity:_run_op(ctx, post_done)
  local utility = self._utility
