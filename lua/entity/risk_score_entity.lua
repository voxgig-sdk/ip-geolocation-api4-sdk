...
local RiskScoreEntity = {}
RiskScoreEntity.__index = RiskScoreEntity


function RiskScoreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RiskScoreEntity)
  self._name = "risk_score"
  self._client = client
...
function RiskScoreEntity:get_name()
  return self._name
end


function RiskScoreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RiskScoreEntity.new(self._client, opts)
end
...
function RiskScoreEntity:mark_deleted()
  self._deleted = true
end


function RiskScoreEntity:deleted()
  return true == self._deleted
end


function RiskScoreEntity:data_set(args)
  if args ~= nil then
...
function RiskScoreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RiskScoreEntity:match_set(args)
  if args ~= nil then
...
function RiskScoreEntity:match_get()
...
function RiskScoreEntity:stream(action, args, callopts)
...
---@param reqmatch RiskScoreLoadMatch
---@param ctrl? table
---@return RiskScore
---@return string? err
function RiskScoreEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RiskScoreEntity:_run_op(ctx, post_done)
  local utility = self._utility
