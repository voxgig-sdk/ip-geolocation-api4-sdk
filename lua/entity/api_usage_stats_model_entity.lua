...
local ApiUsageStatsModelEntity = {}
ApiUsageStatsModelEntity.__index = ApiUsageStatsModelEntity


function ApiUsageStatsModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiUsageStatsModelEntity)
  self._name = "api_usage_stats_model"
  self._client = client
...
function ApiUsageStatsModelEntity:get_name()
  return self._name
end


function ApiUsageStatsModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiUsageStatsModelEntity.new(self._client, opts)
end
...
function ApiUsageStatsModelEntity:mark_deleted()
  self._deleted = true
end


function ApiUsageStatsModelEntity:deleted()
  return true == self._deleted
end


function ApiUsageStatsModelEntity:data_set(args)
  if args ~= nil then
...
function ApiUsageStatsModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiUsageStatsModelEntity:match_set(args)
  if args ~= nil then
...
function ApiUsageStatsModelEntity:match_get()
...
function ApiUsageStatsModelEntity:stream(action, args, callopts)
...
---@param reqmatch ApiUsageStatsModelLoadMatch
---@param ctrl? table
---@return ApiUsageStatsModel
---@return string? err
function ApiUsageStatsModelEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ApiUsageStatsModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ApiUsageStatsModelEntity
