...
local MxnEntity = {}
MxnEntity.__index = MxnEntity


function MxnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MxnEntity)
  self._name = "mxn"
  self._client = client
...
function MxnEntity:get_name()
  return self._name
end


function MxnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MxnEntity.new(self._client, opts)
end
...
function MxnEntity:mark_deleted()
  self._deleted = true
end


function MxnEntity:deleted()
  return true == self._deleted
end


function MxnEntity:data_set(args)
  if args ~= nil then
...
function MxnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MxnEntity:match_set(args)
  if args ~= nil then
...
function MxnEntity:stream(action, args, callopts)
...
---@param reqmatch MxnLoadMatch
---@param ctrl? table
---@return Mxn
---@return string? err
function MxnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MxnEntity:_run_op(ctx, post_done)
  local utility = self._utility
