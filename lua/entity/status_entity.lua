...
local StatusEntity = {}
StatusEntity.__index = StatusEntity


function StatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StatusEntity)
  self._name = "status"
  self._client = client
...
function StatusEntity:get_name()
  return self._name
end


function StatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StatusEntity.new(self._client, opts)
end
...
function StatusEntity:mark_deleted()
  self._deleted = true
end


function StatusEntity:deleted()
  return true == self._deleted
end


function StatusEntity:data_set(args)
  if args ~= nil then
...
function StatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StatusEntity:match_set(args)
  if args ~= nil then
...
function StatusEntity:match_get()
...
function StatusEntity:stream(action, args, callopts)
...
---@param reqmatch StatusLoadMatch
---@param ctrl? table
---@return Status
---@return string? err
function StatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
