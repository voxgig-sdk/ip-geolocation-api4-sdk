-- IpGeolocationApi4 SDK Reverse entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReverseEntity = {}
ReverseEntity.__index = ReverseEntity


function ReverseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReverseEntity)
  self._name = "reverse"
  self._client = client
...
function ReverseEntity:get_name()
  return self._name
end


function ReverseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReverseEntity.new(self._client, opts)
end
...
function ReverseEntity:mark_deleted()
  self._deleted = true
end


function ReverseEntity:deleted()
  return true == self._deleted
end


function ReverseEntity:data_set(args)
  if args ~= nil then
...
function ReverseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReverseEntity:match_set(args)
  if args ~= nil then
...
function ReverseEntity:match_get()
...
function ReverseEntity:stream(action, args, callopts)
...
---@param reqmatch ReverseLoadMatch
---@param ctrl? table
---@return Reverse
---@return string? err
function ReverseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReverseEntity:_run_op(ctx, post_done)
  local utility = self._utility
