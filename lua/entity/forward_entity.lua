...
local ForwardEntity = {}
ForwardEntity.__index = ForwardEntity


function ForwardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ForwardEntity)
  self._name = "forward"
  self._client = client
...
function ForwardEntity:get_name()
  return self._name
end


function ForwardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ForwardEntity.new(self._client, opts)
end
...
function ForwardEntity:mark_deleted()
  self._deleted = true
end


function ForwardEntity:deleted()
  return true == self._deleted
end


function ForwardEntity:data_set(args)
  if args ~= nil then
...
function ForwardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ForwardEntity:match_set(args)
  if args ~= nil then
...
function ForwardEntity:match_get()
...
function ForwardEntity:stream(action, args, callopts)
...
---@param reqmatch ForwardLoadMatch
---@param ctrl? table
---@return Forward
---@return string? err
function ForwardEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ForwardEntity:_run_op(ctx, post_done)
  local utility = self._utility
