-- IpGeolocationApi4 SDK Ipn2 entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local Ipn2Entity = {}
Ipn2Entity.__index = Ipn2Entity


function Ipn2Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Ipn2Entity)
  self._name = "ipn2"
  self._client = client
...
function Ipn2Entity:get_name()
  return self._name
end


function Ipn2Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Ipn2Entity.new(self._client, opts)
end


function Ipn2Entity:data_set(args)
  if args ~= nil then
...
function Ipn2Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Ipn2Entity:match_set(args)
  if args ~= nil then
...
function Ipn2Entity:stream(action, args, callopts)
...
---@param reqmatch Ipn2LoadMatch
---@param ctrl? table
---@return Ipn2
---@return string? err
function Ipn2Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
function Ipn2Entity:_run_op(ctx, post_done)
  local utility = self._utility
