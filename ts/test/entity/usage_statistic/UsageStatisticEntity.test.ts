...
describe('UsageStatisticEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'usage_statistic.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API__TEST_USAGE_STATISTIC_ENTID JSON to run live')
      return
    }
    ...
    const usage_statistic_ref01_data_dt0 = await usage_statistic_ref01_ent.load(usage_statistic_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_GEOLOCATION_API__TEST_USAGE_STATISTIC_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'IP_GEOLOCATION_API__TEST_USAGE_STATISTIC_ENTID': idmap,
    'IP_GEOLOCATION_API__TEST_LIVE': 'FALSE',
    'IP_GEOLOCATION_API__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['IP_GEOLOCATION_API__TEST_USAGE_STATISTIC_ENTID']
