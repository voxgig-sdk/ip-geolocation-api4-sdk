...
describe('StatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'status.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_GEOLOCATION_API4_TEST_STATUS_ENTID']
  ...
  idmap = env['IP_GEOLOCATION_API4_TEST_STATUS_ENTID']
