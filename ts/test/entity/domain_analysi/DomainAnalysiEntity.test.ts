...
describe('DomainAnalysiEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'domain_analysi.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API__TEST_DOMAIN_ANALYSI_ENTID JSON to run live')
      return
    }
    ...
    domain_analysi_ref01_data = await domain_analysi_ref01_ent.create(domain_analysi_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_GEOLOCATION_API__TEST_DOMAIN_ANALYSI_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'IP_GEOLOCATION_API__TEST_DOMAIN_ANALYSI_ENTID': idmap,
    'IP_GEOLOCATION_API__TEST_LIVE': 'FALSE',
    'IP_GEOLOCATION_API__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['IP_GEOLOCATION_API__TEST_DOMAIN_ANALYSI_ENTID']
