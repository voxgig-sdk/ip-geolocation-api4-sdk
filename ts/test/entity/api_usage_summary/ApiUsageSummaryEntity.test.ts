...
describe('ApiUsageSummaryEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'api_usage_summary.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API__TEST_API_USAGE_SUMMARY_ENTID JSON to run live')
      return
    }
    ...
    const api_usage_summary_ref01_data_dt0 = await api_usage_summary_ref01_ent.load(api_usage_summary_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_GEOLOCATION_API__TEST_API_USAGE_SUMMARY_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'IP_GEOLOCATION_API__TEST_API_USAGE_SUMMARY_ENTID': idmap,
    'IP_GEOLOCATION_API__TEST_LIVE': 'FALSE',
    'IP_GEOLOCATION_API__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['IP_GEOLOCATION_API__TEST_API_USAGE_SUMMARY_ENTID']
