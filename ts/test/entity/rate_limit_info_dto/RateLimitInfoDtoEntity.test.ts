...
describe('RateLimitInfoDtoEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'rate_limit_info_dto.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API__TEST_RATE_LIMIT_INFO_DTO_ENTID JSON to run live')
      return
    }
    ...
    const rate_limit_info_dto_ref01_data_dt0 = await rate_limit_info_dto_ref01_ent.load(rate_limit_info_dto_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IP_GEOLOCATION_API__TEST_RATE_LIMIT_INFO_DTO_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'IP_GEOLOCATION_API__TEST_RATE_LIMIT_INFO_DTO_ENTID': idmap,
    'IP_GEOLOCATION_API__TEST_LIVE': 'FALSE',
    'IP_GEOLOCATION_API__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['IP_GEOLOCATION_API__TEST_RATE_LIMIT_INFO_DTO_ENTID']
