# ip-geolocation-api4-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpGeolocationApi4 SDK as
two agent tools — `ip-geolocation-api4_list` and `ip-geolocation-api4_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-api4-mcp)
make build

# 2. Provide credentials via the environment
export IP_GEOLOCATION_API4_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-geolocation-api4 \
  -- /absolute/path/to/ip-geolocation-api4-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-geolocation-api4-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-geolocation-api4_list: first page of records
{ "entity": "advanced" }
{ "entity": "advanced", "query": { } }

// ip-geolocation-api4_load: one record by id
{ "entity": "advanced", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-api4-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-geolocation-api4 \
     -- "$PWD"/dist/*/ip-geolocation-api4-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-geolocation-api4_list` and `ip-geolocation-api4_load` tools now appear
   in new sessions. Ask the agent to *"list advanced using ip-geolocation-api4"*
   and it calls `ip-geolocation-api4_list` with `{"entity":"advanced"}`.
...
```sh
export IP_GEOLOCATION_API4_APIKEY=sk_live_xxx            # API key
export IP_GEOLOCATION_API4_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-geolocation-api4_list` tool
...
### Call the `ip-geolocation-api4_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-geolocation-api4_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-geolocation-api4_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_GEOLOCATION_API4_APIKEY` | API key sent with every request. |
| `IP_GEOLOCATION_API4_BASE` | Optional override of the API base URL. |
...
```sh
./ip-geolocation-api4-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-geolocation-api4_load","arguments":{"entity":"advanced","query":{"id":1}}}}'
```
