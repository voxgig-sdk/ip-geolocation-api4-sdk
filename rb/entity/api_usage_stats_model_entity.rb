...
class ApiUsageStatsModelEntity
  ...
  def make
    opts = @_entopts.dup
    ApiUsageStatsModelEntity.new(@_client, opts)
  end
  ...
  # @return [ApiUsageStatsModel, Hash] the current ApiUsageStatsModel data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ApiUsageStatsModel fields)
  def match_get
  ...
  #
  # @param reqmatch [ApiUsageStatsModelLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ApiUsageStatsModel.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ApiUsageStatsModel, Hash] the loaded ApiUsageStatsModel; raises IpGeolocationApi4Error on failure
