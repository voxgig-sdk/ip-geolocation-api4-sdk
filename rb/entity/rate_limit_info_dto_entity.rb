...
class RateLimitInfoDtoEntity
  ...
  def make
    opts = @_entopts.dup
    RateLimitInfoDtoEntity.new(@_client, opts)
  end
  ...
  # @return [RateLimitInfoDto, Hash] the current RateLimitInfoDto data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of RateLimitInfoDto fields)
  def match_get
  ...
  #
  # @param reqmatch [RateLimitInfoDtoLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RateLimitInfoDto.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RateLimitInfoDto, Hash] the loaded RateLimitInfoDto; raises IpGeolocationApi4Error on failure
