...
class CacheManagementEntity
  ...
  def make
    opts = @_entopts.dup
    CacheManagementEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpGeolocationApi4Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CacheManagement, Hash] the current CacheManagement data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpGeolocationApi4Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CacheManagement fields)
  def match_get
  ...
  #
  # @param reqmatch [CacheManagementLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CacheManagement.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CacheManagement, Hash] the loaded CacheManagement; raises IpGeolocationApi4Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpGeolocationApi4Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CacheManagementRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [CacheManagement, Hash] the removed CacheManagement; raises IpGeolocationApi4Error on failure
