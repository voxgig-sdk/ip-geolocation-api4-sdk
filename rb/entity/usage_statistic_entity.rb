...
class UsageStatisticEntity
  ...
  def make
    opts = @_entopts.dup
    UsageStatisticEntity.new(@_client, opts)
  end
  ...
  # @return [UsageStatistic, Hash] the current UsageStatistic data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of UsageStatistic fields)
  def match_get
  ...
  #
  # @param reqmatch [UsageStatisticLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.UsageStatistic.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [UsageStatistic, Hash] the loaded UsageStatistic; raises IpGeolocationApi4Error on failure
