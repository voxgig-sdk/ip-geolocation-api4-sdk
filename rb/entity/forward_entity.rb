...
class ForwardEntity
  ...
  def make
    opts = @_entopts.dup
    ForwardEntity.new(@_client, opts)
  end
  ...
  # @return [Forward, Hash] the current Forward data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Forward fields)
  def match_get
  ...
  #
  # @param reqmatch [ForwardLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Forward.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Forward, Hash] the loaded Forward; raises IpGeolocationApi4Error on failure
