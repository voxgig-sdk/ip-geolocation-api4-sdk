...
class ApiUsageSummaryEntity
  ...
  def make
    opts = @_entopts.dup
    ApiUsageSummaryEntity.new(@_client, opts)
  end
  ...
  # @return [ApiUsageSummary, Hash] the current ApiUsageSummary data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ApiUsageSummary fields)
  def match_get
  ...
  #
  # @param reqmatch [ApiUsageSummaryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ApiUsageSummary.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ApiUsageSummary, Hash] the loaded ApiUsageSummary; raises IpGeolocationApi4Error on failure
