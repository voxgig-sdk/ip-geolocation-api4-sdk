...
class StatusEntity
  ...
  # @return [Status, Hash] the current Status data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Status fields)
  def match_get
  ...
  #
  # @param reqmatch [StatusLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Status.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Status, Hash] the loaded Status; raises IpGeolocationApi4Error on failure
