...
class RiskScoreEntity
  ...
  def make
    opts = @_entopts.dup
    RiskScoreEntity.new(@_client, opts)
  end
  ...
  # @return [RiskScore, Hash] the current RiskScore data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of RiskScore fields)
  def match_get
  ...
  #
  # @param reqmatch [RiskScoreLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RiskScore.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RiskScore, Hash] the loaded RiskScore; raises IpGeolocationApi4Error on failure
