...
class IpReputationEntity
  ...
  def make
    opts = @_entopts.dup
    IpReputationEntity.new(@_client, opts)
  end
  ...
  # @return [IpReputation, Hash] the current IpReputation data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of IpReputation fields)
  def match_get
  ...
  #
  # @param reqmatch [IpReputationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IpReputation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IpReputation, Hash] the loaded IpReputation; raises IpGeolocationApi4Error on failure
