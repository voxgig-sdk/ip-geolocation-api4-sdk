...
class IpInfoV0Entity
  ...
  def make
    opts = @_entopts.dup
    IpInfoV0Entity.new(@_client, opts)
  end
  ...
  # @return [IpInfoV0, Hash] the current IpInfoV0 data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of IpInfoV0 fields)
  def match_get
  ...
  #
  # @param reqmatch [IpInfoV0LoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IpInfoV0.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IpInfoV0, Hash] the loaded IpInfoV0; raises IpGeolocationApi4Error on failure
