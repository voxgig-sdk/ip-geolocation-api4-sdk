...
module IpGeolocationApi4Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IpGeolocationApi4Context.new({}, nil)
    end
    op = ctx.op || IpGeolocationApi4Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IpGeolocationApi4Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IpGeolocationApi4Error) ? err.msg : err.to_s
    ...
    sdk_err = IpGeolocationApi4Error.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IpGeolocationApi4Error)
