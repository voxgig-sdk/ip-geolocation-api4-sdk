...
class ReverseEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_REVERSE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    assert !reverse_ref01_data_dt0_loaded.nil?
...
def reverse_basic_setup(extra)
  ...
  entid_env_raw = ENV["IPGEOLOCATIONAPI__TEST_REVERSE_ENTID"]
