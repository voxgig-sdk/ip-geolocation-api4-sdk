...
class PaddleControllerEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = paddle_controller_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "paddle_controller." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    paddle_controller_ref01_ent = client.PaddleController(nil)
    ...
    paddle_controller_ref01_data_dt0_loaded = paddle_controller_ref01_ent.load(paddle_controller_ref01_match_dt0, nil)
    assert !paddle_controller_ref01_data_dt0_loaded.nil?

  end
end

def paddle_controller_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "paddle_controller", "PaddleControllerTestData.json")
