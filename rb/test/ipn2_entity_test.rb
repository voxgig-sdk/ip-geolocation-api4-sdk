...
class Ipn2EntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = ipn2_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "ipn2." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_IPN__ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ipn2_ref01_data = nil
    if ipn2_ref01_data_raw.length > 0
...
def ipn2_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ipn2", "Ipn2TestData.json")
  ...
  entid_env_raw = ENV["IPGEOLOCATIONAPI__TEST_IPN__ENTID"]
