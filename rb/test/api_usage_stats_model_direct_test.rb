...
class ApiUsageStatsModelDirectTest < Minitest::Test
  def test_direct_load_api_usage_stats_model
    setup = api_usage_stats_model_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-api_usage_stats_model", setup[:live] ? "live" : "unit")
    if _should_skip
...
def api_usage_stats_model_direct_setup(mockres)
