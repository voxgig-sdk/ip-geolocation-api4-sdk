...
class ApiUsageSummaryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_API_USAGE_SUMMARY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def api_usage_summary_basic_setup(extra)
  ...
  entid_env_raw = ENV["IP_GEOLOCATION_API4_TEST_API_USAGE_SUMMARY_ENTID"]
