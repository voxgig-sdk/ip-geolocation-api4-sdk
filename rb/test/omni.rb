...
module IpGeolocationApi4Omni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if IpGeolocationApi4Omni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = IpGeolocationApi4Omni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| IpGeolocationApi4Omni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || IpGeolocationApi4Omni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = IpGeolocationApi4Spec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = IpGeolocationApi4Error.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { IpGeolocationApi4Omni.errify(err) }
