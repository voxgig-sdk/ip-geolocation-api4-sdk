...
class IpGeolocationApi4MakeRequest
{
    public static function call(IpGeolocationApi4Context $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new IpGeolocationApi4Response(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new IpGeolocationApi4Response($fetched);
        } else {
