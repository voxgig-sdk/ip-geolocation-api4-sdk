...
// IpGeolocationApi4 SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IpGeolocationApi4Omni::makeStructRunner(
    ...
    {
        return IpGeolocationApi4Omni::objmodel(IpGeolocationApi4Omni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return IpGeolocationApi4Omni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return IpGeolocationApi4Omni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IpGeolocationApi4Omni::structfix($now, Struct::undef()))
                        !== json_encode(IpGeolocationApi4Omni::structfix($val, Struct::undef()))) {
