...
module IpGeolocationApi4Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpGeolocationApi4Response.new({})
    ...
    if fetch_err
      ...
      response = IpGeolocationApi4Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpGeolocationApi4Response.new(fetched)
    else
