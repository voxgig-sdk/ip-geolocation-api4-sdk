...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = IpGeolocationApi4Omni.make_runner(TEST_JSON_FILE, IpGeolocationApi4SDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    IpGeolocationApi4Error.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : IpGeolocationApi4Omni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = IpGeolocationApi4Operation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
