...
type ApiUsageSummaryEntity struct {
	name    string
...
func NewApiUsageSummaryEntity(client *core.IpGeolocationApi4SDK, entopts map[string]any) *ApiUsageSummaryEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ApiUsageSummaryEntity) GetName() string { return e.name }
...
func (e *ApiUsageSummaryEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ApiUsageSummaryEntity) Deleted() bool {
	return e.deleted
}


func (e *ApiUsageSummaryEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewApiUsageSummaryEntity(e.client, opts)
}

func (e *ApiUsageSummaryEntity) Data(args ...any) any {
...
func (e *ApiUsageSummaryEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an ApiUsageSummary; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ApiUsageSummaryEntity) DataTyped(data ...ApiUsageSummary) ApiUsageSummary {
	if len(data) > 0 {
		return typedFrom[ApiUsageSummary](e.Data(asMap(data[0])))
	}
	return typedFrom[ApiUsageSummary](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through ApiUsageSummary (all fields
// optional at the wire level).
func (e *ApiUsageSummaryEntity) MatchTyped(match ...ApiUsageSummary) ApiUsageSummary {
	if len(match) > 0 {
		return typedFrom[ApiUsageSummary](e.Match(asMap(match[0])))
	}
	return typedFrom[ApiUsageSummary](e.Match())
}
...
func (e *ApiUsageSummaryEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ApiUsageSummaryEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ApiUsageSummaryLoadMatch and returns an ApiUsageSummary. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *ApiUsageSummaryEntity) LoadTyped(reqmatch ApiUsageSummaryLoadMatch, ctrl map[string]any) (ApiUsageSummary, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return ApiUsageSummary{}, err
	}
	return typedFrom[ApiUsageSummary](res), nil
}



func (e *ApiUsageSummaryEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *ApiUsageSummaryEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ApiUsageSummaryEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *ApiUsageSummaryEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *ApiUsageSummaryEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
