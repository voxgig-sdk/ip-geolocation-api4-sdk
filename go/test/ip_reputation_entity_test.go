...
func TestIpReputationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_IP_REPUTATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ip_reputationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPGEOLOCATIONAPI__TEST_IP_REPUTATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPGEOLOCATIONAPI__TEST_IP_REPUTATION_ENTID"])
