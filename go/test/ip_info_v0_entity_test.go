...
func TestIpInfoV0Entity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_IP_INFO_V__ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		ipInfoV0Ref01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.ip_info_v0", setup.data)))
...
func ip_info_v0BasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPGEOLOCATIONAPI__TEST_IP_INFO_V__ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPGEOLOCATIONAPI__TEST_IP_INFO_V__ENTID"])
