...
func TestReverseEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_REVERSE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func reverseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPGEOLOCATIONAPI__TEST_REVERSE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPGEOLOCATIONAPI__TEST_REVERSE_ENTID"])
