...
import (
	...
	omni "github.com/voxgig-sdk/ip-geolocation-api4-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpGeolocationApi4Error); is {
				out := map[string]any{
					"name":    "IpGeolocationApi4Error",
...
func omniCtx(arg any, client *sdk.IpGeolocationApi4SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpGeolocationApi4SDK); is {
			client = live
