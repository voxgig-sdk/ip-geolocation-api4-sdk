...
func TestDomainReputationV1DtoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_DOMAIN_REPUTATION_V__DTO_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		domainReputationV1DtoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.domain_reputation_v1_dto", setup.data)))
...
func domain_reputation_v1_dtoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPGEOLOCATIONAPI__TEST_DOMAIN_REPUTATION_V__DTO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPGEOLOCATIONAPI__TEST_DOMAIN_REPUTATION_V__DTO_ENTID"])
