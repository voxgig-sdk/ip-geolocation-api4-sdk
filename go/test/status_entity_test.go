...
func TestStatusEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_STATUS_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		statusRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.status", setup.data)))
		var statusRef01Data map[string]any
		if len(statusRef01DataRaw) > 0 {
			statusRef01Data = core.ToMapAny(statusRef01DataRaw[0][1])
		}
		...
		statusRef01MatchDt0 := map[string]any{}
		statusRef01DataDt0Loaded, err := statusRef01Ent.Load(statusRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if statusRef01DataDt0Loaded == nil {
...
func statusBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IP_GEOLOCATION_API4_TEST_STATUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IP_GEOLOCATION_API4_TEST_STATUS_ENTID"])
