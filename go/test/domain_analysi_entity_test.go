...
func TestDomainAnalysiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_DOMAIN_ANALYSI_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		domainAnalysiRef01Data = core.ToMapAny(domainAnalysiRef01DataResult)
...
func domain_analysiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPGEOLOCATIONAPI__TEST_DOMAIN_ANALYSI_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPGEOLOCATIONAPI__TEST_DOMAIN_ANALYSI_ENTID"])
