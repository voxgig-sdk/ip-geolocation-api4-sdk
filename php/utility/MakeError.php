...
class IpGeolocationApi4MakeError
{
    public static function call(?IpGeolocationApi4Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpGeolocationApi4Context([], null);
        }
        $op = $ctx->op ?? new IpGeolocationApi4Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpGeolocationApi4Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpGeolocationApi4Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpGeolocationApi4Error('', $msg, $ctx);
        ...
        if ($err instanceof IpGeolocationApi4Error) {
