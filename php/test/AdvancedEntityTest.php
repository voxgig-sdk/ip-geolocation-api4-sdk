...
class AdvancedEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpGeolocationApi4SDK::test(null, null);
        $ent = $testsdk->Advanced(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = advanced_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "advanced." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_ADVANCED_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $advanced_ref01_data = null;
        if (count($advanced_ref01_data_raw) > 0) {
...
function advanced_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/advanced/AdvancedTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["advanced01", "advanced02", "advanced03"] as $k) {
    ...
    $entid_env_raw = getenv("IP_GEOLOCATION_API4_TEST_ADVANCED_ENTID");
