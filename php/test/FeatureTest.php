...
// IpGeolocationApi4 SDK feature test
//
...
class FtCtrl extends IpGeolocationApi4Control
{
...
class FtClient
{
    ...
    private IpGeolocationApi4Utility $utility;

    public function __construct(IpGeolocationApi4Utility $utility, array $options)
    {
    ...
    public function get_utility(): IpGeolocationApi4Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpGeolocationApi4Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpGeolocationApi4Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpGeolocationApi4Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpGeolocationApi4Config::make_feature($name);
    ...
    private static function buildUrl(IpGeolocationApi4Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpGeolocationApi4Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpGeolocationApi4Response($fetched);
        }
    ...
    private function fail(IpGeolocationApi4Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpGeolocationApi4Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
