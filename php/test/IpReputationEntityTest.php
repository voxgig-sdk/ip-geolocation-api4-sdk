...
class IpReputationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPGEOLOCATIONAPI__TEST_IP_REPUTATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function ip_reputation_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ip_reputation01", "ip_reputation02", "ip_reputation03"] as $k) {
    ...
    $entid_env_raw = getenv("IPGEOLOCATIONAPI__TEST_IP_REPUTATION_ENTID");
