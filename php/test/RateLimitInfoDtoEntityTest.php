...
class RateLimitInfoDtoEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_RATE_LIMIT_INFO_DTO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function rate_limit_info_dto_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("IP_GEOLOCATION_API4_TEST_RATE_LIMIT_INFO_DTO_ENTID");
