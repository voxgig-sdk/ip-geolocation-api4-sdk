...
class TorEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IpGeolocationApi4SDK::test(null, null);
        $ent = $testsdk->Tor(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = tor_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "tor." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IP_GEOLOCATION_API4_TEST_TOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function tor_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/tor/TorTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["tor01", "tor02", "tor03"] as $k) {
    ...
    $entid_env_raw = getenv("IP_GEOLOCATION_API4_TEST_TOR_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["IP_GEOLOCATION_API4_TEST_TOR_ENTID"]);
