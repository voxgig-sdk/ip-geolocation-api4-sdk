...
// IpGeolocationApi4 SDK retry feature
...
class IpGeolocationApi4RetryFeature extends IpGeolocationApi4BaseFeature
{
    ...
    public function init(IpGeolocationApi4Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpGeolocationApi4Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpGeolocationApi4Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpGeolocationApi4Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
