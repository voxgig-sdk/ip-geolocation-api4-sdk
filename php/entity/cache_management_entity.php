...
// IpGeolocationApi4 SDK CacheManagement entity
...
class CacheManagementEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CacheManagementEntity($this->_client, $opts);
    }

    /**
     * @param CacheManagement|array $args CacheManagement data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpGeolocationApi4Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CacheManagement|array The current CacheManagement data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CacheManagement fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpGeolocationApi4Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CacheManagement fields).
     */
    ...
    /**
     * Load a single CacheManagement.
     *
     * @param CacheManagementLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CacheManagementLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CacheManagement|array The loaded CacheManagement as an assoc-array at the
     *   SDK boundary; throws IpGeolocationApi4Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpGeolocationApi4Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * Remove an CacheManagement matching the given criteria.
     *
     * @param CacheManagementRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; CacheManagementRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CacheManagement|array The removed CacheManagement as an assoc-array at the
     *   SDK boundary; throws IpGeolocationApi4Error on failure (item-5 convention).
     */
